...
export class AIService {
	...
	public initializeAiModels() {
		if (this.modelsInitialized) {
			console.log(
				'AI models already initialized. Call reinitializeModels() to force re-initialization.'
			);
			return;
		}
		...
		if (userProfile?.preferences.aiEnabled) {
			...
			const ollamaModel =
				userProfile.preferences.ollamaEmbeddingModel || 'nomic-embed-text'; // Default embedding model
			const ollamaChatModel =
				userProfile.preferences.ollamaChatModel || 'llama3';
			...
			if (ollamaApiEndpoint) {
				try {
					...
					console.log(
						'Ollama models initialized with endpoint:',
						ollamaApiEndpoint
					);
				} catch (error) {
			...
			const geminiEmbeddingModel =
				userProfile.preferences.geminiEmbeddingModel || 'embedding-001'; // Default embedding model
			const geminiChatModel =
				userProfile.preferences.geminiChatModel || 'gemini-pro';
	...
	private getActiveChatModel(): Ollama | ChatGoogleGenerativeAI | null {
		...
		const preferredProvider =
			useAppStore.getState().userProfile?.preferences.aiProviderPreference;
	...
	public async getOntologySuggestions(
		...
		console.log(
			'AIService: getOntologySuggestions called with context:',
			context,
			'and ontology:',
			existingOntology
		);
	...
	public async getAutoTags(
		...
		console.log(
			'AIService: getAutoTags called for note:',
			noteTitle,
		...
		try {
			...
			return Array.isArray(parsedResponse)
				? parsedResponse.filter(tag => typeof tag === 'string')
				: [];
		} catch (error) {
	...
	public async getEmbeddingVector(text: string): Promise<number[]> {
		...
		console.log(
			'AIService: getEmbeddingVector called for text snippet:',
			text.substring(0, 100)
		);
		try {
			const vector = await embeddingModel.embedQuery(text);
			console.log(
				'AI Response (Embedding Vector):',
				vector
					? `Vector of dimension ${vector.length}`
			...
			if (
				...
				console.error(
					`Embedding model might not be available or endpoint is incorrect. Model: ${embeddingModel.constructor.name}`
				);
			}
			return [];
		}
		...
		if (!currentPrefs || !prevPrefs) {
			...
			if (currentPrefs && !prevPrefs) {
				...
				if (currentPrefs.aiEnabled) {
					console.log(
						'AI settings potentially changed (profile loaded), reinitializing AI models if AI enabled.'
					);
