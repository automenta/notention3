...
describe('FolderService', () => {
  ...
  beforeEach(() => {
    ...
    mockFolders = [
      { id: 'folder-root1', name: 'Root Folder 1', noteIds: ['note1'], children: ['folder-child1'], createdAt: new Date(), updatedAt: new Date() },
      { id: 'folder-child1', name: 'Child Folder 1', parentId: 'folder-root1', noteIds: ['note2'], children: [], createdAt: new Date(), updatedAt: new Date() },
      { id: 'folder-root2', name: 'Root Folder 2', noteIds: [], children: [], createdAt: new Date(), updatedAt: new Date() },
    ];
    mockNotes = [
        { id: 'note1', title: 'Note 1', folderId: 'folder-root1', content:'', tags:[], values:{}, fields:{}, status:'draft', createdAt: new Date(), updatedAt: new Date() },
        { id: 'note2', title: 'Note 2', folderId: 'folder-child1', content:'', tags:[], values:{}, fields:{}, status:'draft', createdAt: new Date(), updatedAt: new Date() },
        { id: 'note3', title: 'Note 3 (unfiled)', content:'', tags:[], values:{}, fields:{}, status:'draft', createdAt: new Date(), updatedAt: new Date() },
    ];

    (DBService.getAllFolders as vi.Mock).mockResolvedValue([...mockFolders]);
    (DBService.getFolder as vi.Mock).mockImplementation(async (id: string) => mockFolders.find(f => f.id === id) || null);
  ...
  describe('createFolder', () => {
    it('should create a new root folder', async () => {
      ...
      const savedArg = (DBService.saveFolder as vi.Mock).mock.calls[0][0] as Folder;
    ...
    it('should create a new child folder and update parent', async () => {
      ...
      (DBService.getFolder as vi.Mock).mockResolvedValueOnce(mockFolders.find(f => f.id === parentId));
      ...
      const newFolderArg = (DBService.saveFolder as vi.Mock).mock.calls.find(c => c[0].id === newFolder.id)![0] as Folder;
      const parentUpdateArg = (DBService.saveFolder as vi.Mock).mock.calls.find(c => c[0].id === parentId)![0] as Folder;
  ...
  describe('updateFolder', () => {
    it('should update folder name', async () => {
        ...
        (DBService.getFolder as vi.Mock).mockResolvedValueOnce(mockFolders.find(f => f.id === folderId));
    ...
    it('should move folder to new parent and update old/new parents', async () => {
        ...
        (DBService.getFolder as vi.Mock)
            .mockResolvedValueOnce(mockFolders.find(f => f.id === folderToMoveId)!) // folder-child1
            .mockResolvedValueOnce(mockFolders.find(f => f.id === 'folder-root1')!)   // old parent: folder-root1
            .mockResolvedValueOnce(mockFolders.find(f => f.id === newParentId)!);      // new parent: folder-root2

        await FolderService.updateFolder(folderToMoveId, { parentId: newParentId });

        expect(DBService.saveFolder).toHaveBeenCalledTimes(3); // folder, old parent, new parent

        const movedFolderCall = (DBService.saveFolder as vi.Mock).mock.calls.find(c => c[0].id === folderToMoveId)!;
        expect(movedFolderCall[0].parentId).toBe(newParentId);

        const oldParentCall = (DBService.saveFolder as vi.Mock).mock.calls.find(c => c[0].id === 'folder-root1')!;
        expect(oldParentCall[0].children).not.toContain(folderToMoveId);

        const newParentCall = (DBService.saveFolder as vi.Mock).mock.calls.find(c => c[0].id === newParentId)!;
  ...
  describe('deleteFolder', () => {
    it('should delete a folder, its children, and unassign notes', async () => {
      ...
      (DBService.getNote as vi.Mock)
        .mockImplementation(async (id: string) => mockNotes.find(n => n.id === id) || null);
      ...
      expect(DBService.saveNote).toHaveBeenCalledWith(expect.objectContaining({ id: noteInRootId, folderId: undefined }));
      expect(DBService.saveNote).toHaveBeenCalledWith(expect.objectContaining({ id: noteInChildId, folderId: undefined }));
    });
  });
  ...
  describe('addNoteToFolder', () => {
    it('should add a note to a folder and update note folderId', async () => {
        ...
        expect((DBService.saveNote as vi.Mock).mock.calls[0][0].folderId).toBe(folderId);

        expect(DBService.saveFolder).toHaveBeenCalledTimes(1);
        expect((DBService.saveFolder as vi.Mock).mock.calls[0][0].noteIds).toContain(noteId);
    });
    ...
    it('should move a note from an old folder to a new folder', async () => {
        ...
        expect(DBService.saveNote).toHaveBeenCalledTimes(1);
        expect((DBService.saveNote as vi.Mock).mock.calls[0][0].folderId).toBe(newFolderId);

        // Save folder called for old and new folder
        expect(DBService.saveFolder).toHaveBeenCalledTimes(2);
        const oldFolderSaveCall = (DBService.saveFolder as vi.Mock).mock.calls.find(c => c[0].id === oldFolderId)!;
        expect(oldFolderSaveCall[0].noteIds).not.toContain(noteId);

        const newFolderSaveCall = (DBService.saveFolder as vi.Mock).mock.calls.find(c => c[0].id === newFolderId)!;
  ...
  describe('removeNoteFromFolder', () => {
    it('should remove a note from a folder and clear note folderId', async () => {
        ...
        expect(DBService.saveNote).toHaveBeenCalledTimes(1);
        expect((DBService.saveNote as vi.Mock).mock.calls[0][0].folderId).toBeUndefined();

        expect(DBService.saveFolder).toHaveBeenCalledTimes(1);
        expect((DBService.saveFolder as vi.Mock).mock.calls[0][0].noteIds).not.toContain(noteId);
    });
  });
