...
export class FolderService {
  ...
  static async createFolder(name: string, parentId?: string): Promise<Folder> {
    ...
    if (parentId) {
      ...
      if (parentFolder) {
        parentFolder.children = [...(parentFolder.children || []), newFolder.id];
  ...
  static async updateFolder(id: string, updates: Partial<Folder>): Promise<Folder | null> {
    ...
    if (oldParentId !== actualNewParentId) {
      ...
      if (actualNewParentId) {
        ...
        if (newParent) {
          newParent.children = [...new Set([...(newParent.children || []), id])]; // Ensure uniqueness
   ...
   */
  static async deleteFolder(id: string, allFoldersMap: { [folderId: string]: Folder }): Promise<void> {
   ...
   */
  static async addNoteToFolder(noteId: string, folderId: string): Promise<void> {
    const note = await DBService.getNote(noteId);
   ...
   */
  static async removeNoteFromFolder(noteId: string, folderId: string): Promise<void> {
    const note = await DBService.getNote(noteId);
    const folder = await DBService.getFolder(folderId);
